...
#include "ca3types.h"
	...
	const float eps_ma = 0.5f; // % stable, if < 1.0 Hz
	struct RESULT {
		RUN run;
		int mode; 
	...
	{int mode = 0; // run single backend, 0 for CPU, 1 for GPU
		...
		//{int run = RUN(CURRENT);
			...
			// amount of neurons
			//int n = 10;	// functional tests
			int n = 40;	// functional tests
			//int n = 100;	// functional tests
			//int n = 743;  // example
			//int n = 74366;	// CA3 mouse

			create_PC(sim, neuronGroup, n);					EXPECT_GE(neuronGroup[PC], 0);		
			create_BC(sim, neuronGroup, 100);				EXPECT_GE(neuronGroup[BC], 0);

			// Recurrent pricipal cells
			// use same weights and probability as the CA3 model
			connect_PC_PC(sim, connGroup, neuronGroup);		EXPECT_GE(connGroup[PC][PC], 0);

			// Recurrent interneurons
			connect_BC_BC(sim, connGroup, neuronGroup);		EXPECT_GE(connGroup[BC][BC], 0);

			// PC with Interneurons
			connect_PC_BC(sim, connGroup, neuronGroup, 5.0f, 0.99019f);		EXPECT_GE(connGroup[PC][BC], 0);
			connect_BC_PC(sim, connGroup, neuronGroup, 1.45f, 0.30f);		EXPECT_GE(connGroup[BC][PC], 0);
			...
			case RUN(POISSON): {
				...
				sim->connect(g0, neuronGroup[PC], "one-to-one", RangeWeight(0.0f, wt, 40.0f), 1.0f, // 0.0250664662231983f,   
			...
			case RUN(CURRENT): {
				...
				sim->connect(g0, neuronGroup[PC], "one-to-one", RangeWeight(0.0f, wt, 40.0f), 0.75, // 0.0250664662231983f,   
					RangeDelay(1, 2), RadiusRF(-1.0), SYN_PLASTIC, 0.553062478f, 0.0f);
				//RangeDelay(1), RadiusRF(-1.0), SYN_PLASTIC, 0.553062478f, 0.0f);
			...
			case RUN(POISSON):
				break;
			case RUN(CURRENT):
				sim->setExternalCurrent(g0, 100.0f);  // 1100ms: 10,10,20 Hz
				break;
			default:
				;
			}
			...
			SpikeMonitor* spkMon2 = sim->setSpikeMonitor(neuronGroup[BC], "DEFAULT");
            ...
            // complete first chunk 
						...
						if (i > 0)
						{
							for (int j = 0; j < 5; j++) {
								...
								float af2 = spkMon2->getPopMeanFiringRate();
								if (af > 5.0f && af2 > 1.5) {  // Hz  
									...
									//printf("%5d ms %s %s MeanFiringRate(PC, BC) (%.1f, %.1f) Hz  MA(%.0f) = %.1f Hz\n", t, mode == GPU_MODE ? "GPU" : "CPU", run_names[run], af, af2, n_ma, ma);
			...
			delete sim;
			EXPECT_NEAR(results[mode][run].ma, 14.5f, 1.0f);  // MA(5) of AF in Hz
			EXPECT_NEAR(results[mode][run].spikes, 2000, 1000);  // 1.5s 
		}
		...
		if (runs > 1) {
			...
			EXPECT_NEAR(results[mode][POISSON].spikes, results[mode][CURRENT].spikes, 1000);  // 1.5s 
		}
	}
