...
TEST(CsGen, synfire4periodstat) {
	...
	FILE *txt = fopen("results/synfire4periodstat.txt", "w");
	FILE *csv = fopen("results/synfire4periodstat.csv", "w"); // for alpha and E
	...
	for (int iter = 0; iter < NIter; iter++) {
...

		{
			...
			unsigned int spikes = 0;
			carlsim->getSimSummary(spikes);
			//EXPECT_NEAR(spikes, 27697, 5500);  // only for 1000
		...
		fprintf(txt, "%d. (seed:%d)  E(once):%.0f - E(period):%.0f =  %.0f\n", iter, seed, mean_once, mean_periodical, mean_once - mean_periodical);
		fprintf(txt, "%d. (seed:%d) Spikes(once):%d - Spikes(period):%d =  %d\n", iter, seed, spikes_once.back(), spikes_periodical.back(), spikes_once.back() - spikes_periodical.back());

		fprintf(csv, "%d;%d;%d;%d\n", seed, spikes_once.back(), spikes_periodical.back(), spikes_once.back() - spikes_periodical.back());
	...
	fprintf(txt, "SD(once):%.1f, SD(period):%.1f,  SD(delta):%.1f\n", SD_o, SD_p, SD_d);
...
TEST(CsGen, synfire4export) {

	// Random  Spikes
	// file -> 26830  (random connections generated by CARLsimGUI)

	// see also below carlsim->printEntrails(..);
	/*
		case CONFIG_SNN:
			compileSNN();
		case COMPILED_SNN:
		...
		case PARTITIONED_SNN:
			generateRuntimeSNN();
	*/
	CARLsim::Params()[KERNEL_FEATURES_PARAM] = KERNEL_EXPORT_CONNECTOM | KERNEL_CONNECTION_CHECKSUMS;

	//CARLsim* carlsim = new CARLsim("synfire4loopB", CPU_MODE, USER, 0, 42);
	CARLsim* carlsim = new CARLsim("synfire4loopB", CPU_MODE, USER, 0, 3404);
	/*
	1;26629;26097;532
	2;26723;26938;-215
	4;26887;27191;-304
	7;26575;26100;475
	667; 26526; 24636; 1890
	3404; 26687; 30372; -3685
	3656; 26684; 23573; 3111
	*/
...
#include "synfire4loopB/monitors.h"
	...
	carlsim->setupNetwork();
	#include "synfire4loopB/deletes.h"
	state = carlsim->getCARLsimState();
	EXPECT_EQ(state, SETUP_STATE);
	...
	*/
	...
	// start export, write binary, .. 
	
	// network name, .. 

	// groups -> generate binary dump of NeuronGroupConfigs  .dat


	// connections  -> generate binary dump of ConnectionGroupConfigs  .dat


	// monitors -> generate XML 

	// spike generators -> generate XML 

	/*
	const int length = 1000000;
	char buffer[length];
	carlsim->printEntrails(buffer, length, 0, 1);
	FILE *f = fopen("entrails.txt", "w"); 
	fputs(buffer, f);
	fclose(f);
	*/
