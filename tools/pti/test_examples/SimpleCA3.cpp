...
public:
    ...
    static const int TRANSIENT_SECONDS = 2;
    ...
    SimpleCA3Experiment(const SimMode simMode, const LoggerMode verbosity, const int deviceID) : simMode(simMode), verbosity(verbosity), deviceID(deviceID) {}
...
private:
    typedef struct SimpleCA3Network {
        ...
        static SimpleCA3Network * const createNetwork(const std::vector<double> parameters, const SimMode simMode, const LoggerMode verbosity, const int deviceID) {
