classdef CobaReader < handle
    ...
    %
    % Example usage:
    % >> cR = CobaReader('results/c_group1.dat');
    % >> cValues = cR.readValues();
    %
    ...
    properties (Hidden, Access = private)
        ...
        max_coba_mon_size;   % maximum number of neurons recorded in a group
    ...
    %% PUBLIC METHODS
    methods
        function obj = CobaReader(cobaFile, storeValues, errorMode)
            ...
            [~,~,fileExt] = fileparts(cobaFile);
            if strcmpi(fileExt,'')
                obj.throwError(['Parameter cobaFile must be a file ' ...
        ...
        function max_coba_mon_size = getMaxCobaMonSize(obj)
            max_coba_mon_size = obj.max_coba_mon_size;
        end
        ...
        function nValues = readValues(obj)
            ...
            % length.
            %
            % Returns a struct with 4 fields: AMPA, NMDA, GABAa, GABAb. Each field is 
           ...
           if obj.storeValues
            ...
            data = fread(obj.fileId,[24 Inf],'uint8=>uint8');
            
            t = typecast(reshape(data(1:4,:),[],1),'uint32');
            nId = typecast(reshape(data(5:8,:),[],1),'uint32');
            ampa = typecast(reshape(data(9:12,:),[],1),'single');
            nmda = typecast(reshape(data(13:16,:),[],1),'single');
            gaba_a = typecast(reshape(data(17:20,:),[],1),'single');
            gaba_b = typecast(reshape(data(21:24,:),[],1),'single');
            
            dataSize = [min(obj.max_coba_mon_size, prod(obj.grid3D)), (obj.stimLengthMs+1)];
            nValues.ampa = reshape(ampa, dataSize);
            nValues.nmda = reshape(nmda, dataSize);
            nValues.gaba_a = reshape(gaba_a, dataSize);
            nValues.gaba_b = reshape(gaba_b, dataSize);
    ...
    methods (Hidden, Access = private)
        ...
        function openFile(obj)
            ...
            obj.max_coba_mon_size = fread(obj.fileId, 1, 'int32');
